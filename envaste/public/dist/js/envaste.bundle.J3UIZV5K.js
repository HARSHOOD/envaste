(() => {
  // ../envaste/envaste/public/js/transaction.js
  erpnext.TransactionController = class TransactionController extends erpnext.taxes_and_totals {
    ...
    make_quality_inspection() {
      ...
      (async () => {
        const promises = this.frm.doc.items.map(async (item) => {
          if (!item.quality_inspection) {
            let sample_size = await frappe.db.get_value("Item Group", item.item_group, "custom_number_of_samples_of_quality_control");
            ...
            dialog_items.df.data.push({
              ...
              "serial_no": item.serial_no,
              "batch_no": item.batch_no,
              "sample_size": item.sample_quantity || sample_size.message.custom_number_of_samples_of_quality_control
            });
            dialog_items.grid.refresh();
          }
        });
        await Promise.all(promises);
        const data2 = dialog.fields_dict.items.df.data;
        if (!data2.length) {
...
//# sourceMappingURL=envaste.bundle.J3UIZV5K.js.map
